    ...
    <>
      <div className="border-t border-border pt-10">
        <div className="flex flex-wrap items-center gap-3">
          ...
          {/* 生成分享卡片 — 朋友圈/小红书/微博图卡
              走 /api/card/[slug] 动态生成 PNG,新窗口打开(右键/长按保存) */}
          <a
            href={`/api/card/${article.slug}`}
            target="_blank"
            rel="noopener noreferrer"
            className="share-btn"
            aria-label="生成分享卡片(图片)"
            title="生成分享卡片 · 长按/右键保存到相册"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path
                strokeLinecap="round"
      ...
      {copied && (
        <div
          className="fixed inset-x-0 bottom-24 z-[80] flex justify-center pointer-events-none"
          role="status"
          aria-live="polite"
        >
...
function WeChatShare({ url, title }: { url: string; title: string }) {
  ...
  // Esc 关闭 + 打开时把焦点移到弹层(简单做法:聚焦第一个可聚焦元素)
  useEffect(() => {
    if (!open) return;
