...
export default function ArticlePage({ params }: { params: { slug: string } }) {
  ...
  return (
    <>
      ...
      {/* 文章头 */}
      <article className="max-w-reading mx-auto px-6 pt-8 pb-12">
        <div className="text-center mb-10">
          <div className="flex items-center justify-center gap-3 mb-6">
            <Seal variant="gold">{article.dynasty}</Seal>
            <span className="text-xs text-ink-mute">资治通鉴 · {article.volume}</span>
            <span className="text-xs text-ink-mute">·</span>
            <span className="text-xs text-ink-mute">第 {article.episode} 期</span>
          </div>
          <h1 className="text-3xl md:text-[44px] font-bold text-ink leading-[1.25] mb-6">
            {article.title}
          </h1>
          {article.subtitle && (
          ...
          <div className="flex items-center justify-center gap-5 text-xs text-ink-mute">
